...
public class ADGoodsBooking extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.elad_goods_booking);
		// Log.i("ADGoodsBooking", "onCreate");

		Buttonfdate = (Button) findViewById(R.id.booking_selecta);
		Buttonfdate.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				Calendar calendar = Calendar.getInstance(Locale.CHINA);
				DatePickerDialog dpd = new DatePickerDialog(ADGoodsBooking.this, dsl, calendar.get(Calendar.YEAR),
						calendar.get(Calendar.MONTH), calendar.get(Calendar.DAY_OF_MONTH));
				dpd.show();
			}
		});

		Buttonftime = (Button) findViewById(R.id.booking_selectb);
		Buttonftime.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				Calendar calendar = Calendar.getInstance(Locale.CHINA);
				new TimePickerDialog(ADGoodsBooking.this, new TimePickerDialog.OnTimeSetListener() {
		...
		edittexta = (EditText) findViewById(R.id.booking_edittexta);
		edittextb = (EditText) findViewById(R.id.booking_edittextb);
		edittextc = (EditText) findViewById(R.id.booking_edittextc);
		edittextd = (EditText) findViewById(R.id.booking_edittextd);
		edittexte = (EditText) findViewById(R.id.booking_edittexte);
		edittextf = (EditText) findViewById(R.id.booking_edittextf);

		Button button_submit = (Button) findViewById(R.id.booking_button_submit);
		...
		if (doValidate()) {
			...
			new Thread() {
				public void run() {
					...
					strParam.append("&Booking_content=");
					...
					InputStream is = AppliedMethod.doPostSubmit(BasicProperties.HTTP_URL_BOOKING, strParam.toString());
	...
	private boolean doValidate() {
		if ("".equals(edittexta.getText().toString().trim())) {
			Toast.makeText(getApplicationContext(), this.getString(R.string.booking_textviewa_yz), Toast.LENGTH_SHORT)
					.show();
			return false;
		}
		if ("".equals(edittextb.getText().toString().trim())) {
			Toast.makeText(getApplicationContext(), this.getString(R.string.booking_textviewb_yz), Toast.LENGTH_SHORT)
					.show();
			return false;
		}
		if ("".equals(edittextc.getText().toString().trim())) {
			Toast.makeText(getApplicationContext(), this.getString(R.string.booking_textviewc_yz), Toast.LENGTH_SHORT)
					.show();
			return false;
		}
		if ("".equals(edittextd.getText().toString().trim())) {
			Toast.makeText(getApplicationContext(), this.getString(R.string.booking_textviewd_yz), Toast.LENGTH_SHORT)
					.show();
			return false;
		}
		if ("".equals(edittexte.getText().toString().trim())) {
			Toast.makeText(getApplicationContext(), this.getString(R.string.booking_textviewe_yz), Toast.LENGTH_SHORT)
					.show();
			return false;
		}
		if ("".equals(edittextf.getText().toString().trim())) {
			Toast.makeText(getApplicationContext(), this.getString(R.string.booking_textviewf_yz), Toast.LENGTH_SHORT)
					.show();
			return false;
		}
		return true;
			...
			try {
				...
				while (eventCode != XmlPullParser.END_DOCUMENT) {
					...
					case XmlPullParser.START_TAG:
						if (BasicProperties.XML_TAG67.equals(nodeName)) {
							String AddBooking = xpp.nextText();
							if (AddBooking.equals("1")) {
								msg.what = 1;
								msg.obj = this.getString(R.string.booking_request_right);
								handler.sendMessage(msg);

								// Intent intent = new Intent();
								// intent.setClass(ADGoodsBooking.this,QuestionResultActivity.class);
								// startActivity(intent);
								overridePendingTransition(R.anim.push_left_in, R.anim.push_left_out);
								ADGoodsBooking.this.finish();
							} else {
								msg.what = 2;
								msg.obj = this.getString(R.string.booking_request_error);
								handler.sendMessage(msg);
							}
						}
						break;
	...
	private DatePickerDialog.OnDateSetListener dsl = new DatePickerDialog.OnDateSetListener() {
		...
		public void onDateSet(DatePicker datePicker, int year, int month, int dayOfMonth) {
			edittextd.setText(year + "-" + fillGap(month + 1) + "-" + fillGap(dayOfMonth));
		}
	};
