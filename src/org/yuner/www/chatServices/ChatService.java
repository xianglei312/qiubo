...
import org.yuner.www.chatter.ChatListviewAdapter;
...
public class ChatService extends Service {
	...
	public void onDestroy() {
		...
		// localIntent.setClass(this, ChatService.class);
	...
	public void sendMyMessage(String st0) {
		if (mCurType == GlobalMsgTypes.msgFromFriend) {
			ChatEntity ent0 = new ChatEntity(mCurType, mMyUserInfo, mFriendId, st0);
			NetworkService.getInstance().sendUpload(mCurType, ent0.toString());
			newMsgArrive(ent0.toString(), true);
			//newMsgArrive(ent0.toString()+GlobalStrings.entityDivider+" ", true);
		}
	}
	...
	public void newMsgArrive(String str0, boolean isSelf,String fanyi) {
		ChatEntity msgEntity = new ChatEntity(str0);
		
		msgEntity.settvTranslate(fanyi);//单独插入翻译数据
		...
		if (ChatActivity.getIsActive()) {
			...
			if (mCurType == GlobalMsgTypes.msgFromFriend) {
				mChatActv.updateListviewHistory(msgs, isSelf, mCurType,
						FriendListInfo.getFriendListInfo().getNameFromId(mFriendId));
			}
		}
	}
