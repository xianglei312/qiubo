...
public class FriendRequestService extends Service {
	...
	public void processFriendRequest(String msg0) {
		...
		MainTabMsgPage.getInstance().onUpdateById(TabMsgItemEntity.FrdReqNotifId, mRequester.getAvatarId(),
				mRequester.getName(), mRequester.getName() + R.string.want_to_add, ChatEntity.genDate(), true);
		...
		FrdRequestNotifActivity.newNotification(FrdReqNotifItemEntity.mTypeFrdReq, mRequester.getId(),
				mRequester.getAvatarId(), mRequester.getName(), mRequester.getName() + R.string.want_to_add,
				ChatEntity.genDate(), mRequester.toString());
		if (FrdRequestNotifActivity.getInstance() != null
				&& FrdRequestNotifActivity.getInstance().getIsCurPage() == true) {
	...
	public void processFriendRequestResponse(String msg0) {
		...
		if (reqEnt0.isAceepted()) {
			MainTabMsgPage.getInstance().onUpdateById(TabMsgItemEntity.FrdReqNotifId, mRequestee.getAvatarId(),
					mRequestee.getName(), mRequestee.getName() + R.string.accepted_your_request, ChatEntity.genDate(),
					true);
		} else {
			MainTabMsgPage.getInstance().onUpdateById(TabMsgItemEntity.FrdReqNotifId, mRequestee.getAvatarId(),
					mRequestee.getName(), mRequestee.getName() + R.string.declined_your_request, ChatEntity.genDate(),
					true);
		}
		...
		String msg11;
		if (!reqEnt0.isAceepted()) {
			msg11 = mRequestee.getName() + R.string.accepted_your_request;
		} else {
			msg11 = mRequestee.getName() + R.string.declined_your_request;
		...
		FrdRequestNotifActivity.newNotification(FrdReqNotifItemEntity.mTypeFrdReqResult, mRequestee.getId(),
				mRequestee.getAvatarId(), mRequestee.getName(), msg11, ChatEntity.genDate(), mRequestee.toString());
		if (FrdRequestNotifActivity.getInstance() != null
				&& FrdRequestNotifActivity.getInstance().getIsCurPage() == true) {
			FrdRequestNotifActivity.getInstance().onUpdateView();
		}
